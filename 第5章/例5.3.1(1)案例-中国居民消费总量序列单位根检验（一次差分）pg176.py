...
X = np.column_stack((T, Y_d1_lag1, Y_d2_lag1))
# 对整个数组删除有NaN值的每一行
X = X[~np.any(np.isnan(X), axis=1)]  # 1 按行； 0 按列
Y_d2 = Y_d2.iloc[:-3]
if len(Y_d2) != len(X):
    print(f'Y数据长度={len(Y_d2)}')
    print(f'X数据长度={len(X)}')
    exit('回归数据Y和X长度不匹配')
##########################################################################
# 进行多元线性回归，可以替换Y和X
...
##########################################################################
model = sm.OLS(Y_d2, sm.add_constant(X))  # 用add_constant加入常数项
fit = model.fit()  # not using HC0,HC1 etc.
print(fit.summary(title='Y_d2', yname='Y_d2',
                  xname=['const', 'T', 'Y_d1_lag1', 'Y_d2_lag1']))
print('model3 H0: 存在单位根。请查看Y_d1_lag1的显著性看是否拒绝H0')
mylmtest.LM_TEST_lag3(Y_d2, X)

# 模型2：常数项+Y滞后项
X = np.column_stack((Y_d1_lag1, Y_d2_lag1))
X = X[~np.any(np.isnan(X), axis=1)]  # 1 按行； 0 按列
model = sm.OLS(Y_d2, sm.add_constant(X))  # 用add_constant加入常数项
fit = model.fit(use_t=True)  # not using HC0,HC1 etc.
print(fit.summary(title='Y_d2', yname='Y_d2',
                  xname=['const', 'Y_d1_lag1', 'Y_d2_lag1']))
...
mylmtest.LM_TEST_lag3(Y_d2, X, nocons=True)
